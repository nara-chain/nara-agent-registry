...
pub fn set_bio(
    ...
    if info.lamports() == 0 {
        ...
        // Write discriminator + bio.
        let mut data = info.try_borrow_mut_data()?;
        data[..8].copy_from_slice(&AgentBio::DISCRIMINATOR);
        let bio_bytes = bio.as_bytes();
        data[8..12].copy_from_slice(&(bio_bytes.len() as u32).to_le_bytes());
        data[12..12 + bio_bytes.len()].copy_from_slice(bio_bytes);
    } else {
        ...
        // Overwrite bio content (discriminator stays intact).
        let mut data = info.try_borrow_mut_data()?;
        let bio_bytes = bio.as_bytes();
        data[8..12].copy_from_slice(&(bio_bytes.len() as u32).to_le_bytes());
        data[12..12 + bio_bytes.len()].copy_from_slice(bio_bytes);
